...
$_SESSION['url']='http://localhost/cphrms/admin/create_doctor.php';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if(isset($_FILES['uploadfile1']['name'])){
        ...
        if (!in_array($file_type, $allowed_types)) {
            ...
            $image1 = $_FILES["uploadfile1"]["name"];
            $imagefile_name1=uniqid() . '-' . $image1;
    
            $tempname1 = $_FILES["uploadfile1"]["tmp_name"];
            $folder1 = "../superadmin/image/signature/" . $imagefile_name1;
	...
	if (empty($_POST['position'])) {
		...
		$posit = mysqli_real_escape_string($connection, trim($_POST['position']));
	}
    ...
    if (empty($_POST['department_name'])) {
		...
		$departmentd = $_POST['department_name'];
	}
	$query1 = "SELECT department_name FROM department WHERE department_name='$departmentd' and hospital_id='$hid'";
		$qrun4 = mysqli_query($connection,$query1);
	if(mysqli_num_rows($qrun4)!=1){
	$errors[]="Department Dont exist on Hospital ";
    $errors['sqlerror3']="Department Dont exist on Hospital ";

}else{
		$department=$departmentd;

	}
	$sql5="SELECT department_id from department where department_name='$department'";
	$qrun5 = mysqli_query($connection,$sql5);
    $usertypes1 = mysqli_fetch_array($qrun5);
	$depid=$usertypes1['department_id'];
...
$sql = "INSERT INTO user (user_id,name,email,password,role) VALUES ('$uid','$fn','$ea','$pw','$rl')";
		...
		if ($r) {
			// If it ran OK.
		    $sql1 = "INSERT INTO doctor(user_id,bdate,gender,cstatus,hnum,street,brgy,mun,prov,phone,photo,doctor_id,doctor_position,department_id,hospital_id,active,signature) VALUES ('$uid','$bd','$g','$s','$hn','$st','$bgy','$mun','$pv','$pn','$imagefile_name','$docid','$posit','$depid','$hosp_id','ACTIVE','$imagefile_name1')";
...
<?php  
				...
				if(!empty($success['success'])){
                    ?>
                <div style="text-align:center;background-color:green;color:white; display:block; padding:10px;font-size:14px;transform: translateY(-20px);margin-bottom: 20px;" class="red-text"><?php echo $success['success'];?><a style="color:white;"href="create_doctor.php">Create another Doctor</a></div>
                </div>
				<?php
                }else{
                ?>  
...
<label></label>
  ...
  <div style="flex: 1;">
	 ...
	 <p>Doctor ID: </p><input class="form-control" type="text" name="doctor_id" size="15" maxlength="20" value="<?php  echo generateDKey(); ?>"readonly="readonly" />
				</div>
				</div>
				<label></label>
				<div class="mb-3">
  ...
  <div style="flex: 1;">
	...
	<option value="">Select Doctor's Position</option>
...
<div class="mb-3">
  ...
  <div style="flex: 1;">
<p>Signature</p>
<input class="form-control" type="file" accept="image/*" name="uploadfile1" value="" required/>
</div>
<div style="flex: 1; margin-left: 10px;">
