...
$_SESSION['url']='http://localhost/cphrms/admin/create_patient.php';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($_POST['gname'])) {
		$errors[] = 'You forgot to enter your Guardians name.';
	} else {
		$gn = $_POST['gname'];
		$gn=encryptthis($gn,$key);
	}
	if (empty($_POST['gphone'])) {
		$errors[] = 'You forgot to enter your Guardians phone.';
	} else {
		$gp = $_POST['gphone'];
		$gp=encryptthis($gp,$key);
	}
	
	if (empty($_POST['gaddress'])) {
		$errors[] = 'You forgot to enter your Guardians address.';
	} else {
		$ga = $_POST['gaddress'];
		$ga=encryptthis($ga,$key);
	}
	...
	if (!$_POST['btype']){
		$errors[]="No Blood Type";
	
		}else{
		$bt=$_POST['btype'];
			$bt=encryptthis($bt,$key);
		}
    if (!$_POST['role'] ){
    $errors[]="No Role";
	...
	if (empty($_POST['gemail'])) {
		$errors[] = 'You forgot to enter your Guardians email.';
	} else {
		$gem= $_POST['gemail'];
		$sql5="SELECT * from user where email='$gem' LIMIT 1";
		$qrun5 = mysqli_query($connection,$sql5);
		$usertypes = mysqli_fetch_array($qrun5);

		if(mysqli_num_rows($qrun5)==1){
			if(($usertypes['role']=='SuperAdmin')||($usertypes['role']=='Admin')||($usertypes['role']=='Doctor')||($usertypes['role']=='Laboratory')){
            $errors['sqlerror3']="You cannot use this email as your guardian email address";
			$errors[]="You cannot use this email as your guardian email address";
...
$sql = "INSERT INTO user (user_id,name,email,password,role) VALUES ('$uid','$fn','$ea','$pw','$rl')";
		 ...
		 $sql2 = "INSERT INTO patient (user_id,bdate,bplace,gender,cstatus,btype,hnum,street,brgy,mun,prov,phone,photo,gname,gphone,gemail,gaddress) VALUES ('$uid','$bd','$bp','$g','$s','$bt','$hn','$st','$bgy','$mun','$pv','$pn','$imagefile_name','$gn','$gp','$ge','$ga' )";
		...
		if ($r2) {
			...
			// Print a message:
            $success['success']="Patient Created Successfully";
			echo "<script>window.location.href='success_patient.php?id=$uid';</script>";
			...
			// Public message:
            $errors['sqlerror']="Failed to Create Patient";
...
<div style="width: 1500px;height:100%" class="form-style-10">
<h1>Create Patient<span></span></h1>
<form action="create_patient.php"  method="POST" enctype="multipart/form-data">
<?php  
	...
	<p> Full Name:</p>    
            ...
            <input class="form-control" type="text" name="name"size="15" maxlength="40" pattern="[a-zA-Z\s]+" placeholder="First Name/Middle Name/Last Name" value="<?php if (isset($_POST['name'])) echo $_POST['name']; ?>" required/>
  ...
  <div style="flex: 1;">
	<p>Birth Date: </p>
	 ...
	 <input class="form-control" type="text" name="bplace" size="15" maxlength="40" pattern="[a-zA-Z\s]+" placeholder="Birth Place" value="<?php if (isset($_POST['bplace'])) echo $_POST['bplace']; ?>" required/>
...
<div class="section">Address Information</div>
  ...
  <div style="flex: 1;">
	...
	<input class="form-control" type="text" name="gname" size="15" maxlength="40" placeholder="Guardian's Name" pattern="[a-zA-Z\s]+" value="<?php if (isset($_POST['gname'])) echo $_POST['gname']; ?>" required/>
	
	</div>
	<div style="flex: 1; margin-left: 10px;">

	<p>Guardian's Phone Number:</p>
	<input class="form-control" type="tel" name="gphone" size="15" pattern="(^(09|\+63)(\d){9}$)" placeholder="09123456789" value="<?php if (isset($_POST['gphone'])) echo $_POST['gphone']; ?>" required/>
	...
	<input class="form-control" type="email" name="gemail" size="15" maxlength="40" placeholder="Guardian's Email" value="<?php if (isset($_POST['gemail'])) echo $_POST['gemail']; ?>" required/>
	...
	<input class="form-control" type="text" name="gaddress" size="15" maxlength="40" placeholder="Guardian's Address" value="<?php if (isset($_POST['gaddress'])) echo $_POST['gaddress']; ?>" required/>
			</div>	
...
<script>
    function checker(){
        var result= confirm('Are you sure you want to create patient?');
        if (result==false){
