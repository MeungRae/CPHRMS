...
$url='http://localhost/cphrms/admin/edit_laboratory.php?id=';
...
$sql="SELECT * from laboratory where user_id='$student_id'";
...
$sql2="UPDATE laboratory set active='$active' where user_id='$uid'";
$r2 = mysqli_query($connection, $sql2);

if($r2){
    header("Location:search_laboratory.php");
}else{
...
<h1>Edit Laboratory Active Status<span></span></h1>
<form action="edit_laboratory.php"  method="POST" enctype="multipart/form-data">
<?php  
...
<script>
    function checker(){
        var result= confirm('Are you sure you want to update this laboratory active status?');
        if (result==false){
